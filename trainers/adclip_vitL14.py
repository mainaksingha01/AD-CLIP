...
class domain_projector(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear1 = nn.ModuleList(nn.Linear(1024,512) for _ in range (24))
        self.linear2 = nn.ModuleList(nn.Linear(512,768) for _ in range (24))
        self.adain=AdaIN()
...
class image_projector(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear = nn.ModuleList(nn.Linear(1024,768) for _ in range (24))
        self.adain=AdaIN()
...
class ADCLIPL14(TrainerXU):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.ADCLIPL14.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.ADCLIPL14.PREC == "fp32" or cfg.TRAINER.ADCLIPL14.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.ADCLIPL14.PREC == "amp" else None
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        prec = self.cfg.TRAINER.ADCLIPL14.PREC
        # alpha_wt = self.alpha
        if prec == "amp":
            with autocast():
